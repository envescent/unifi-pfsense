...
unifi_start()
{
  if checkyesno ${rcvar}; then
    ...
    su unifi -c "/usr/local/bin/java --add-opens=java.base/java.time=ALL-UNNAMED -jar /usr/local/UniFi/lib/ace.jar start" &
    echo $! > $pidfile
...
unifi_stop()
{
  ...
  if [ -f $pidfile ]; then
    ...
    su unifi -c "/usr/local/bin/java -jar /usr/local/UniFi/lib/ace.jar stop" &
